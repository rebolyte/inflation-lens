...
export function formatPrice(num) {
  if (num >= 1000000000000) {
    ...
    const formatted = trillions.toFixed(2);
...
export function parsePrice(priceStr) {
  ...
  if (cleaned.match(/[TtBbMmKk]$/)) {
    const num = parseFloat(cleaned);
    const suffix = cleaned.slice(-1).toUpperCase();

    if (suffix === 'T') return num * 1000000000000;
    if (suffix === 'B') return num * 1000000000;
    if (suffix === 'M') return num * 1000000;
    if (suffix === 'K') return num * 1000;
  }

  const wordMatch = cleaned.match(/^([\d.]+)\s*(trillion|billion|million|thousand)$/i);
  if (wordMatch) {
    const num = parseFloat(wordMatch[1]);
    const suffix = wordMatch[2].toLowerCase();

    if (suffix === 'trillion') return num * 1000000000000;
    if (suffix === 'billion') return num * 1000000000;
    if (suffix === 'million') return num * 1000000;
    if (suffix === 'thousand') return num * 1000;
  }
